...
/**
 * Fluid Viewhelper to create thumbs
 * Created at 10-03-2014
 * @author  Mark Zuurhout <mark.zuurhout@tros.nl>
 */

class Tx_Gallery_ViewHelpers_ThumbViewHelper extends Tx_Fluid_Core_ViewHelper_AbstractViewHelper {
	 ...
	 * @param boolean $crop
	 */
	public function render($imgPath, $width, $height, $crop=NULL){
		$full_dir = dirname($imgPath);
		$gallery_dir = basename($full_dir);
		$thumb_dir = 'fileadmin/gallery/thumbs/';
		$file = basename($imgPath);

		// create thumb if not already exists
		if(!file_exists($thumb_dir.'/'.$gallery_dir.'-thumb-'.$file)) {
			if (null !== $imgPath) {
				if (is_file($imgPath)) {
					$this->setImageFile($imgPath);
					if($crop){
						$this->resample($width,$height*4); // first resize to get better crop result
						$this->crop(0, 0, $width, $height);
					} else {
						$this->resample($width,$height);
					}
					$this->save($thumb_dir.'/'.$gallery_dir.'-thumb-'.$file);
				}
			}
		}
	}
